...
You will see the background cover image stretched properly and the first frame of the video. Do you see the problem? They are not positioned and stretched exactly the same. I think it is because of the aspect ratio of the video. We'll have to consider the aspect ratio of the browser window to set width and height.

My video's aspect ratio is 16:9 so I'll use this code to set the width and height of the video:
...
The trick here is, that the video element is the child of the cover div. If the video starts playing the cover image won't be visible. If not, the cover will be visible. So really simple, no Javascript required.
